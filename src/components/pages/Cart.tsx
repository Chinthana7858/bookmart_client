...
export default function Cart() {
  ...
  const fetchCartItems = async () => {
    try {
      const res = await axios.get(API.GET_CART_ITEMS(userId ?? 0), {
  ...
  if (loading) return <div className="p-8 text-gray-700">Loading cart...</div>;
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto p-6 min-h-screen">
        ...
        {items.length !== 0 ? (
          <div>
            <button
              className="bg-primary hover:bg-primarydark text-white px-8 py-2 rounded-xl cursor-pointer mt-4"
              onClick={() => {
